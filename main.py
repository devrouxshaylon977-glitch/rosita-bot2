...
ACCOUNT_BALANCE = float(os.getenv("ACCOUNT_BALANCE", "1000") or 1000)
...
def do_backtest(candles):
    ...
    for i in range(21, len(candles)-10):
        ...
        if not prev_bull and curr_bull: # bullish cross -> long
            entry=closes[i]; sl=min(c["l"] for c in candles[i-3:i+1])
            risk=entry-sl
            if risk<=0: continue
            tp=entry+risk*2
            total+=1
        ...
        elif prev_bull and not curr_bull: # bearish cross -> short
            entry=closes[i]; sl=max(c["h"] for c in candles[i-3:i+1])
            risk=sl-entry
            if risk<=0: continue
            tp=entry-risk*2
            total+=1
    ...
    return {"trades":total,"wins":wins,"losses":losses,"winrate":wr}
...
async def handle_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if low in ["hi","hello","hey","yo"]:
        await update.message.reply_text("Hey Shay 🫦 👀 I'm here 💕 scan or backtest?"); return
    ...
    if low.startswith("backtest"):
        ...
        await update.message.reply_text(f"Running real backtest Shay 🫦 👀 {tf} EMA 9/21 cross...")
        candles=get_candles("XAU/USD",tf,200)
        res=do_backtest(candles)
        if not res: await update.message.reply_text("Not enough data Shay 🫦 👀 try later 💕"); return
        msg=(f"Done Shay 🫦 👀 Backtest {tf} EMA 9/21 💕\nTrades: {res['trades']}\nWins: {res['wins']} Losses: {res['losses']}\nWinrate: {res['winrate']}%\n"
             f"{'Solid A setup, keep it' if res['winrate']>=55 else 'Meh, risky Shay, filter more'} 🫦 💕")
        await update.message.reply_text(msg); return
    reply=await ask_groq(text_raw, update.effective_chat.id)
    await update.message.reply_text(reply)

async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Hey Shay! It's Rosita 🫦 👀 Try 'backtest 1h' 💕")
    ...
    while True:
        try:
            if BOSS_CHAT_ID.strip():
                ...
                if c4:
                    last=c4[-1]["c"]
                    sig=await ask_groq(f"Scan XAU/USD at {last} for A/B setup 4h/1h/30m/15m/5m. If A/B give Entry/SL/TP else say 'No A/B setup'.", BOSS_CHAT_ID)
